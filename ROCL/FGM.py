...
class FGM:
    ...
    def __init__(self, model,linear, adv_func, budget,
                 dist_metric='l2'):
        """FGM attack.
    ...
    def get_gradient_dgcnn(self, data, target, normalize=True):
        """Generate one step gradient.
        ...
        # forward pass
        #x1,x2,x3,x=self.model(data) ##############################gaile
        #seg_pred = self.linear(x1,x2,x3,x)
        seg_pred=self.model(data)
        seg_pred = seg_pred.permute(0, 2, 1).contiguous()
        loss = self.adv_func(seg_pred.view(-1, 13), target.view(-1, 1).squeeze())
        loss.backward()
        with torch.no_grad():
            grad = data.grad.detach()  # [B, 3, K]
            if normalize:
...
class IFGM(FGM):
    ...
    def __init__(self, model,linear, adv_func, clip_func,
        ...
        Args:
            ...
            dist_metric (str, optional): type of constraint. Defaults to 'l2'.
        """
        super(IFGM, self).__init__(model,linear, adv_func, budget, dist_metric)
    ...
    def attack(self, data, target):
        ...
        for iteration in range(self.num_iter):
            ...
            #perturbation = torch.sign(normalized_grad)
        ...
        with torch.no_grad():
            '''
            logits = self.model(self.linear(pc))
            if isinstance(logits, tuple):
                logits = logits[0]
            pred = torch.argmax(logits, dim=-1)
            success_num = (pred == target).sum().item()
            '''
            feat,_=self.model(pc)#############################gaile
            out=self.linear(feat)
            _, predx = torch.max(out.data, 1)
            success_num = predx.eq(target.data).cpu().sum().item()
...
class IFGM_seg(FGM):
    ...
    def __init__(self, model,linear, adv_func, clip_func,
        ...
        Args:
            ...
            dist_metric (str, optional): type of constraint. Defaults to 'l2'.
        """
        super(IFGM_seg, self).__init__(model,linear, adv_func, budget, dist_metric)
        self.clip_func = clip_func
        self.step_size = step_size
        self.num_iter = num_iter
        self.model = model
    ...
    def attack(self, data, target):
        ...
        for iteration in range(self.num_iter):
            # gradient
            normalized_grad= self.get_gradient_dgcnn(pc, target)
...
class PGD(IFGM):
    ...
    def __init__(self, model,linear, adv_func, clip_func,
        ...
        Args:
            ...
            dist_metric (str, optional): type of constraint. Defaults to 'l2'.
        """
        super(PGD, self).__init__(model,linear, adv_func, clip_func,
                                  budget, step_size, num_iter,
                                  dist_metric)
