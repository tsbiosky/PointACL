...
def load_data_semseg(partition, test_area):
    ...
    train_area_name="Area_1"
    train_idxs, test_idxs = [], []
    for i, room_name in enumerate(room_filelist):
        if test_area_name in room_name:
            test_idxs.append(i)
        else:
            #if train_area_name in room_name:
                train_idxs.append(i)
    if partition == 'train':
...
def load_color_semseg():
    colors = []
    labels = []
    f = open("/home/junxuan/new/BYOL/prepare_data/meta/semseg_colors.txt")
    ...
    for _ in range(0, img_size):
        ...
        for _ in range(0, img_size):
            ...
            if color_index >= 13:
                cv2.imwrite("/home/junxuan/new/BYOL/prepare_data/meta/semseg_colors.png", img, [cv2.IMWRITE_PNG_COMPRESSION, 0])
...
class S3DIS(Dataset):
    def __init__(self, num_points=4096, partition='train', test_area='1',transforms=None,hg=False):
        ...
        self.hg =hg
        if self.hg ==True:
            print('hg data enbable!!!')
            path = '/mnt/ssd/home/junxuan/data/hf_S3DIS.npz'
    ...
    def __getitem__(self, item):
        ...
        if self.partition == 'train':
            ...
            if self.transforms is not None:
                ...
                pointcloud_ = self.transforms(pointcloud_)
                pointcloud= points_sampler(pointcloud, self.num_points)
                pointcloud_ = points_sampler(pointcloud_, self.num_points)
            #print(pointcloud.shape,pointcloud_.shape)
        seg = torch.LongTensor(seg)
        if self.transforms is not None:
            if self.hg==True:
                return pointcloud,pointcloud_,self.hg_data[item][:self.num_points],seg
            if self.hg==False:
                return pointcloud,pointcloud_, seg
        else:
