...
from BYOL.data.data import ModelNet40npz,ModelNet10
...
def test(model,Linear,testloader,attacker,args):
    ...
    clean_correct = reduce_value(torch.tensor(clean_correct), average=False)  # 预测正确样本个数
    total = reduce_value(torch.tensor(total), average=False)  # 预测正确样本个数
    adv_correct = reduce_value(torch.tensor(adv_correct), average=False)  # 预测正确样本个数
    clean_acc = 100. * clean_correct / total
    adv_acc = 100. * adv_correct / total
...
if __name__ == "__main__":
    ...
    Linear = DistributedDataParallel(Linear,device_ids=[args.local_rank])
    if args.finetune:
        model.train()
    else:
        model.eval()
    ...
    '''
    train_set=ModelNet40npz(partition='train', num_points=args.num_points, normalize=False, transform=train_transforms)
    train_sampler = DistributedSampler(train_set, shuffle=False)
    train_loader = DataLoader(train_set,
                              num_workers=8,
                              batch_size=args.batch_size, shuffle=False, drop_last=True,sampler=train_sampler)
    val_set=ModelNet40npz(partition='test', num_points=args.num_points, normalize=False)
    val_sampler=DistributedSampler(val_set, shuffle=False)
    val_loader = DataLoader(val_set,
                             num_workers=8,
                             batch_size=args.batch_size, shuffle=False, drop_last=False,sampler=val_sampler)
    ...
    for epoch in range(args.epoch):
        ...
        for batch_idx, (inputs,inputs2,target) in enumerate(train_loader):
            ...
            if args.finetune:
                ...
                aug_copy= pc_aug2.clone()
                aug_copy=aug_copy.detach().cpu().numpy()
                np.savez('./adv_train.npz', adv=adv_copy.astype('float32'), aug=aug_copy.astype('float32'))
                '''
        ...
        if device != torch.device("cpu"):
            torch.cuda.synchronize(device)
        val_correct = reduce_value(val_correct, average=False)  # 预测正确样本个数
        val_total =reduce_value(val_total, average=False)  # 预测正确样本个数
        clean_acc = 100. * val_correct / val_total
        if args.local_rank == 0:
        ...
        if clean_acc>best_acc:
            torch.save(Linear.state_dict(), './best_linear.pth')
