...
from BYOL.data.shapenet_loader import Dataset
...
def test(model,Linear,testloader,attacker,args):
    ...
    ori_npz=[]
    adv_npz=[]
    for idx, (pc, label,_,_) in enumerate(testloader):
        with torch.no_grad():
...
if __name__ == "__main__":
    ...
    Linear=nn.Sequential(nn.Linear(args.emb_dims, 55))
    ...
    '''
    train_loader=DataLoader(Dataset(root='/mnt/ssd/home/junxuan/data',split='train', num_points=args.num_points,dataset_name='shapenetcorev2'),
                              num_workers=8,
                              batch_size=args.batch_size, shuffle=True, drop_last=True)
    val_loader = DataLoader(Dataset(root='/mnt/ssd/home/junxuan/data', split='test', num_points=args.num_points,
    ...
    for epoch in range(args.epoch):
        ...
        total = 0
        #if epoch==0:
            #get_emd(model, Linear, train_loader, attacker_test)
        ...
        for idx, (pc, label,_,_) in enumerate(val_loader):
            with torch.no_grad():
                pc, y = pc.float().cuda(non_blocking=True), \
                        label.long().cuda(non_blocking=True).squeeze()
            val_total += y.size(0)
