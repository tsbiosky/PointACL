...
class BasicalModel(pl.LightningModule):
    ...
    def get_gradient(self, aug1, aug2):
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.alpha = 1
        self.k = 5
        B, K_ = pc_aug1.shape[:2]
        #ori = ori.float().cuda().detach()
        #ori = ori.transpose(1, 2).contiguous()  # [B, 3, K]
        ...
        for i in range(num_rounds):
            ...
            grad= \
                self.get_gradient(pc_aug1, pc_aug2)  # [B, 3, K]
            if i % (num_rounds // 5) == 0:
                print('Iteration {}/{},/{}\n'
                      'Point num: {}/{}'.
                      format(i, num_rounds, B,
                             K, K_))

            with torch.no_grad():
                # compute center point
                center = torch.median(pc_aug1, dim=-1)[0]. \
                    clone().detach()  # [B, 3]

                # compute r_i as l2 distance
                r = torch.sum((pc_aug1 - center[:, :, None]) ** 2,
                              dim=1) ** 0.5  # [B, K]

                # compute saliency score
                saliency = -1. * (r ** self.alpha) * \
                           torch.sum((pc_aug1 - center[:, :, None]) * grad,
                                     dim=1)  # [B, K]
                ...
                pc_aug1 = torch.stack([
                    pc_aug1[j, :, idx[j]] for j in range(B)
                ], dim=0)  # [B, 3, K - k]
                pc_aug2 = torch.stack([
                    pc_aug2[j, :, idx[j]] for j in range(B)
                ], dim=0)  # [B, 3, K - k]
                ori = torch.stack([
                    ori[j, :, idx[j]] for j in range(B)
                ], dim=0)  # [B, 3, K - k]

        ###
        y1_online, qz1_online, y2_online, qz2_online, y1_target, z1_target, y2_target, z2_target \
            = self.forward(pc_aug1, ori)
        loss = self.regression_loss(qz1_online, z2_target)
        loss += self.regression_loss(qz2_online, z1_target)
