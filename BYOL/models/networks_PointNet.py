...
class SimCLR_PointNet(PointNet):
    def __init__(self, hparams,pesudo=False):
        super(SimCLR_PointNet,self).__init__(hparams)
        self.hparams = hparams
        self.emb_dims = hparams["emb_dims"]
        self.pesudo=pesudo
        self.build_target_network()
    ...
    def build_target_network(self):
        ...
        if self.pesudo:
            self.fc=nn.Sequential(nn.Linear(self.emb_dims, 40))
            '''
            self.fc40 = nn.Sequential(
                nn.Linear(self.emb_dims, self.emb_dims),
                nn.BatchNorm1d(self.emb_dims),
                nn.ReLU(inplace=True),
                nn.Linear(self.emb_dims, 40)
            )
            self.fc10 = nn.Sequential(
                nn.Linear(self.emb_dims, self.emb_dims),
                nn.BatchNorm1d(self.emb_dims),
                nn.ReLU(inplace=True),
                nn.Linear(self.emb_dims, 10)
            )
            self.fc100 = nn.Sequential(
                nn.Linear(self.emb_dims, self.emb_dims),
                nn.BatchNorm1d(self.emb_dims),
                nn.ReLU(inplace=True),
                nn.Linear(self.emb_dims, 100)
            )
            '''
    def forward(self, pointcloud,fc=False):
        """
        ...
        z: projection
        """
        y = super(SimCLR_PointNet, self).forward(pointcloud)
        z = self.projector(y)
        if fc:
            pred = self.fc(y)
            return y,z,pred
