...
class BasicalModel(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        pc_aug1, pc_aug2,ori_data = batch
        ...
        #print(type(adv_input), type(data), type(pc_aug1))
        for i in range(self.max_iter):
            y1_online, qz1_online, y2_online, qz2_online, y1_target, z1_target, y2_target, z2_target \
                = self.forward(pc_aug1, pc_aug2)
            loss1 = self.regression_loss(qz1_online, z2_target)
            loss1 += self.regression_loss(qz2_online, z1_target)
            grad_outputs = None
            grad = torch.autograd.grad(loss1, pc_aug1, grad_outputs=grad_outputs, only_inputs=True, retain_graph=True)[0]
            ...
            with torch.no_grad():
                ...
                adv_data = self.clip_func(adv_data, ori_data)
        y1_online, qz1_online, y2_online, qz2_online, y1_target, z1_target, y2_target, z2_target \
            = self.forward(pc_aug1, adv_data)
        loss2 = self.regression_loss(qz1_online, z2_target)
        loss2 += self.regression_loss(qz2_online, z1_target)
        y1_online, qz1_online, y2_online, qz2_online, y1_target, z1_target, y2_target, z2_target \
            = self.forward(pc_aug2, adv_data)
        loss3 = self.regression_loss(qz1_online, z2_target)
        loss3 += self.regression_loss(qz2_online, z1_target)
        ...
        return dict(loss=loss, log=log, progress_bar=dict(train_loss=loss,loss1=loss1,loss2=loss2,loss3=loss3))
