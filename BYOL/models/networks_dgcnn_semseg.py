...
class DGCNN_head(nn.Module):
    def __init__(self,hparams):
        super(DGCNN_head, self).__init__()
    ...
    def forward(self,x1,x2,x3,x):
        ...
        x = x.repeat(1, 1, self.hparams['num_points'])  # (batch_size, 1024, num_points)
...
class DGCNN_semseg(nn.Module):
    def __init__(self, hparams,finetune=False):
    ...
    def forward(self, x,vat=False):
        ...
        if vat==True:
            ...
            y = y.repeat(1, 1, num_points)  # (batch_size, 1024, num_points)
            #print(x1.shape, y.shape,x.shape)
            y = torch.cat((y, x1, x2, x3), dim=1)  # (batch_size, 1024+64*3, num_points)
            y = y.max(dim=-1, keepdim=True)[0]  # (batch_size, 256, num_points) -> (batch_size, 256, 1)
            return x,y
        #
        # x = self.conv7(x)  # (batch_size, 1024+64*3, num_points) -> (batch_size, 512, num_points)
        # x = self.conv8(x)  # (batch_size, 512, num_points) -> (batch_size, 256, num_points)
...
class TargetNetwork_DGCNN_Semseg(DGCNN_semseg):
    ...
    def forward(self, pointcloud,vat=False):
        """
        ...
        if vat==False:
            ...
            y,cat = super(TargetNetwork_DGCNN_Semseg, self).forward(pointcloud,vat)
            z = self.projector(y)
            return y, z,cat
